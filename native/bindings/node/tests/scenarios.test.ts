...
function isClose(a: number, b: number): boolean {
  ...
  return (
    Math.abs(a - b) <=
    Math.max(FP_REL_TOL * Math.max(Math.abs(a), Math.abs(b)), FP_ABS_TOL)
  );
}
...
function expectDimsIntEqual(
  label: string,
  expected: DimensionsInt,
  actual: DimensionsInt,
): void {
  expect(actual.width, `${label}.width`).toBe(expected.width);
  expect(actual.height, `${label}.height`).toBe(expected.height);
}

function expectDimsFloatClose(
  label: string,
  expected: DimensionsFloat,
  actual: DimensionsFloat,
): void {
  expect(
    isClose(expected.width, actual.width),
    `${label}.width: expected ${expected.width}, got ${actual.width}`,
  ).toBe(true);
  expect(
    isClose(expected.height, actual.height),
    `${label}.height: expected ${expected.height}, got ${actual.height}`,
  ).toBe(true);
}
...
function expectPointClose(
  ...
  expect(
    isClose(expected.x, actual.x),
    `${label}.x: expected ${expected.x}, got ${actual.x}`,
  ).toBe(true);
  expect(
    isClose(expected.y, actual.y),
    `${label}.y: expected ${expected.y}, got ${actual.y}`,
  ).toBe(true);
}

function expectOptionalDimsIntEqual(
  label: string,
  expected: DimensionsInt | null,
  actual: DimensionsInt | null,
): void {
  if (expected === null) {
...
function expectOptionalDimsFloatClose(
  label: string,
  expected: DimensionsFloat | null,
  actual: DimensionsFloat | null,
): void {
  if (expected === null) {
    expect(actual, `${label} should be null`).toBeNull();
  } else {
...
function expectOptionalPointClose(
  label: string,
  expected: PointFloat | null,
  actual: PointFloat | null,
): void {
  if (expected === null) {
    expect(actual, `${label} should be null`).toBeNull();
  } else {
...
function compareContext(expected: Context, actual: Context): void {
  ...
  expect(actual.contextCreator, "context.contextCreator").toBe(
    expected.contextCreator,
  );
}
...
function compareCanvas(
  ...
  expect(actual.sourceCanvasId, "canvas.sourceCanvasId").toBe(
    expected.sourceCanvasId,
  );
  ...
  expectOptionalDimsIntEqual(
    "canvas.effectiveDimensions",
    expected.effectiveDimensions,
    actual.effectiveDimensions,
  );
  expectOptionalPointClose(
    "canvas.effectiveAnchorPoint",
    expected.effectiveAnchorPoint,
    actual.effectiveAnchorPoint,
  );
  expectOptionalDimsIntEqual(
    "canvas.photositeDimensions",
    expected.photositeDimensions,
    actual.photositeDimensions,
  );
  expectOptionalDimsFloatClose(
    "canvas.physicalDimensions",
    expected.physicalDimensions,
    actual.physicalDimensions,
  );
  expect(
...
function compareFramingDecision(
  expected: FramingDecision,
  actual: FramingDecision,
  expectedId?: string,
): void {
  if (expectedId) {
    expect(actual.id, "framingDecision.id").toBe(expectedId);
  }
  expect(actual.framingIntentId, "framingDecision.framingIntentId").toBe(
    expected.framingIntentId,
  );
  expect(actual.label, "framingDecision.label").toBe(expected.label);
  expectDimsFloatClose(
    "framingDecision.dimensions",
    expected.dimensions,
    actual.dimensions,
  );
  expectPointClose(
    "framingDecision.anchorPoint",
    expected.anchorPoint,
    actual.anchorPoint,
  );
  expectOptionalDimsFloatClose(
    "framingDecision.protectionDimensions",
    expected.protectionDimensions,
    actual.protectionDimensions,
  );
  expectOptionalPointClose(
    "framingDecision.protectionAnchorPoint",
    expected.protectionAnchorPoint,
    actual.protectionAnchorPoint,
  );
  compareCustomAttrs(expected, actual, "framingDecision");
}

function compareCanvasTemplate(
  expected: CanvasTemplate,
  actual: CanvasTemplate,
): void {
  expect(actual.id, "template.id").toBe(expected.id);
  expectDimsIntEqual(
    "template.targetDimensions",
    expected.targetDimensions,
    actual.targetDimensions,
  );
  expect(
  ...
  expect(
    actual.alignmentMethodVertical,
    "template.alignmentMethodVertical",
  ).toBe(expected.alignmentMethodVertical);
  expect(
    actual.alignmentMethodHorizontal,
    "template.alignmentMethodHorizontal",
  ).toBe(expected.alignmentMethodHorizontal);
  expect(
    actual.preserveFromSourceCanvas,
    "template.preserveFromSourceCanvas",
  ).toBe(expected.preserveFromSourceCanvas);
  expectOptionalDimsIntEqual(
    "template.maximumDimensions",
    expected.maximumDimensions,
    actual.maximumDimensions,
  );
  expect(actual.padToMaximum, "template.padToMaximum").toBe(
    expected.padToMaximum,
  );
...
function compareCustomAttrs(
  ...
  for (const key of expectedKeys) {
    expect(
      actualKeys.has(key),
      `${prefix}.customAttrs missing key '${key}'`,
    ).toBe(true);
  }
  for (const key of actualKeys) {
    expect(
      expectedKeys.has(key),
      `${prefix}.customAttrs unexpected key '${key}'`,
    ).toBe(true);
  }
  for (const key of expectedKeys) {
    ...
    if (typeof ev === "number" && typeof av === "number") {
      expect(
        isClose(ev, av),
        `${prefix}.customAttrs['${key}']: expected ${ev}, got ${av}`,
      ).toBe(true);
    } else if (
      ev !== null &&
      av !== null &&
...
function getScenarioPaths(
  ...
  const resultFdlName = config.resultPattern.replace(
    "{variant}",
    variant.fdlBasename,
  );
...
const SCENARIOS: ScenarioConfig[] = [
  {
    ...
    variants: [
      {
        letter: "B",
        fdlBasename: "B_4448x3096_1x_FramingChart",
        contextLabel: "ARRI ALEXA Mini LF",
        canvasLabel: "4.5K LF Open Gate",
      },
      {
        letter: "D",
        fdlBasename: "D_8640x5760_1x_10PercentSafety-FramingChart",
        contextLabel: "Sony VENICE 2 8K",
        canvasLabel: "8.6K 3:2",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    variants: [
      ...
      {
        letter: "E",
        fdlBasename: "E_8640x5760_1x_15PercentSafety-FramingChart",
        contextLabel: "Sony VENICE 2 8K",
        canvasLabel: "8.6K 3:2",
      },
      {
        letter: "F",
        fdlBasename: "F_8640x5760_1x_20PercentSafety-FramingChart",
        contextLabel: "Sony VENICE 2 8K",
        canvasLabel: "8.6K 3:2",
      },
    ],
  ...
  },
  {
    ...
    variants: [
      {
        letter: "A",
        fdlBasename: "A_4448x3096_1-3x_FramingChart",
        contextLabel: "ARRI ALEXA Mini LF",
        canvasLabel: "4.5K LF Open Gate",
      },
      {
      ...
      {
        letter: "C",
        fdlBasename: "C_4448x3096_2x_FramingChart",
        contextLabel: "ARRI ALEXA Mini LF",
        canvasLabel: "4.5K LF Open Gate",
      },
    ],
  ...
  },
  {
    ...
    templateFilename:
      "Scen_7_TopAlignedSource-PreserveProtection-Centered-CANVAS-TEMPLATE.fdl",
    resultPattern: "Scen7-RESULT-{variant}.fdl",
    customSourceDir: "Original_Source_Files",
    variants: [
      {
        letter: "G",
        fdlBasename: "G_5040x3780_1x_TopAligned-FramingChart",
        contextLabel: "RED V-RAPTOR 8K S35",
        canvasLabel: "7K 4:3 2x",
      },
    ],
  },
  {
    ...
    templateFilename:
      "Scen_8_TopAlignedSource-PreserveCanvas-Centered-CANVAS-TEMPLATE.fdl",
  ...
  },
  {
    ...
    templateFilename:
      "Scen_10_FitWidth_MaintainAspectRatio-CANVAS-TEMPLATE.fdl",
    ...
    variants: [
      ...
      {
        letter: "J",
        fdlBasename: "J_4320x3456_2x_FramingChart",
        contextLabel: "RED DSMC2 MONSTRO 8K VV",
        canvasLabel: "4.5K 5:4",
      },
    ],
  },
  {
    ...
    templateFilename:
      "Scen_11_FitHeight_MaintainAspectRatio-CANVAS-TEMPLATE.fdl",
    resultPattern: "{variant}.fdl",
    customSourceDir: "Original_Source_Files",
    variants: [
      {
  ...
  },
  {
    ...
    templateFilename:
      "Scen_14_Preserving-Protection-WithMaxDim-Cropping-TEMPLATE.fdl",
  ...
  },
  {
    ...
    templateFilename:
      "Scen_16_Retention_Of_Relative_Anchors_pad_to_max_top_left.fdl",
  ...
  },
  {
    ...
    templateFilename:
      "Scen_17_Retention_Of_Relative_Anchors_pad_to_max_bottom_left.fdl",
  ...
  },
  {
    ...
    variants: [
      {
        letter: "RED_EPIC",
        fdlBasename: "Test_06_RED_EPIC_Dragon_6K",
        contextLabel: "RED EPIC DRAGON 6K S35",
        canvasLabel: "4K 2:1",
      },
    ],
  },
    ...
    variants: [
      {
        ...
        fdlBasename: "Test_09_Blackmagic_URSA_Cine_17K_65",
        contextLabel: "Blackmagic Design URSA Cine 17K 65",
        canvasLabel: "12K 16:9",
      },
    ],
  },
  {
    ...
    variants: [
      {
        ...
        fdlBasename: "Test_19_ARRI_Alexa_65",
        contextLabel: "ARRI Alexa 65",
        canvasLabel: "6500",
      },
    ],
  },
    ...
    variants: [
      {
        ...
        fdlBasename: "Test_20_ARRI_Alexa_65",
        contextLabel: "ARRI Alexa 65",
        canvasLabel: "6500",
      },
    ],
  },
  {
    ...
    dirName:
      "Scen_26_FitHeight_MaintainAspectRatio_Blackmagic_12K_LF_CustomFraming",
    templateFilename:
      "Scen_11_FitHeight_MaintainAspectRatio-CANVAS-TEMPLATE.fdl",
    ...
    variants: [
      {
        ...
        fdlBasename: "Test_58_Blackmagic_URSA_Cine_12K_LF_CustomFraming",
        contextLabel: "Blackmagic URSA Cine 12K LF",
        canvasLabel: "12K 16:9",
      },
    ],
  },
    ...
    variants: [
      {
        ...
        fdlBasename: "Test_60_RED_EPIC_Dragon_6K_CustomFraming",
        contextLabel: "RED EPIC Dragon 6K",
        canvasLabel: "4K 2:1",
      },
    ],
  },
  {
    ...
    dirName:
      "Scen_28_FitHeight_MaintainAspectRatio_ARRI_Alexa_65_CustomFraming",
    templateFilename:
      "Scen_11_FitHeight_MaintainAspectRatio-CANVAS-TEMPLATE.fdl",
    ...
    variants: [
      {
        ...
        fdlBasename: "Test_62_ARRI_Alexa_65_CustomFraming",
        contextLabel: "ARRI Alexa 65",
        canvasLabel: "6500",
      },
    ],
    ...
    variants: [
      {
        ...
        fdlBasename: "combo_src_topleft_tpl_topcenter_source",
        contextLabel: "Off-Center Source top-left",
        canvasLabel: "4K Canvas - Source top-left",
      },
    ],
    ...
    variants: [
      {
        ...
        fdlBasename: "eff_topleft_to_bottomright_source",
        contextLabel: "Effective Area eff_topleft_to_bottomright",
        canvasLabel: "6K with offset effective area",
      },
    ],
...
describe("Scenario template tests (parity with Python)", () => {
  it.each(TEST_CASES)(
    ...
    ({ config, variant }) => {
      ...
      const framingIntentId =
        config.framingIntentId ?? defaults.framingIntentId;
      ...
      expect(
        existsSync(paths.template),
        `Template not found: ${paths.template}`,
      ).toBe(true);
      expect(
        existsSync(paths.sourceFdl),
        `Source FDL not found: ${paths.sourceFdl}`,
      ).toBe(true);
      expect(
        existsSync(paths.expectedFdl),
        `Expected FDL not found: ${paths.expectedFdl}`,
      ).toBe(true);
      ...
      expect(
        fd,
        `FD with intent '${framingIntentId}' not found`,
      ).not.toBeNull();
      ...
      expect(
        expectedContext,
        `Expected context '${templateLabel}' not found`,
      ).not.toBeNull();
      ...
      expect(
        expectedCanvas,
        `Expected canvas '${expectedCanvasLabel}' not found`,
      ).not.toBeNull();

      const expectedFd = findFD(expectedCanvas!, framingIntentId);
      expect(
        expectedFd,
        `Expected FD with intent '${framingIntentId}' not found`,
      ).not.toBeNull();
      ...
      expect(
        actualContext,
        "Actual context not found in result",
      ).not.toBeNull();
      ...
      expect(
        expectedTemplate,
        `Expected template '${templateLabel}' not found`,
      ).not.toBeNull();
      expect(
        actualTemplate,
        `Actual template '${templateLabel}' not found`,
      ).not.toBeNull();
