...
describe("CanvasTemplate kwargs", () => {
  ...
  it("with optional fields", () => {
    ...
    expect(ct.preserveFromSourceCanvas).toBe(
      GeometryPath.CANVAS_EFFECTIVE_DIMENSIONS,
    );
...
describe("Kwargs vs Builder parity", () => {
  it("FramingIntent kwargs matches builder", () => {
    ...
    // Builder path
    const fdl = new FDL({});
    const builtFi = fdl.addFramingIntent(
      "FI-TEST",
      "Test",
  ...
  it("Context kwargs matches builder", () => {
    ...
    expect(ctx.asDict().context_creator).toEqual(
      builtCtx.asDict().context_creator,
    );
    fdl.close();
  });
});
